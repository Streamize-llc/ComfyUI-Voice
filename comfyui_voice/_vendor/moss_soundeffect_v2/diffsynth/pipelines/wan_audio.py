...
# Vendored ComfyUI-Voice: MOSS-SoundEffect uses the DAC VAE exclusively, so the
# Oobleck VAE is optional. Older diffusers (<0.30) lacks it; degrade gracefully
# to a sentinel class (only used in unreachable isinstance/type-annotation slots).
try:
    from diffusers import AutoencoderOobleck
except ImportError:  # pragma: no cover - DAC path only
    class AutoencoderOobleck:  # type: ignore
        pass
